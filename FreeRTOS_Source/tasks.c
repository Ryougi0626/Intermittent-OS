...
PRIVILEGED_DATA int StackToNVM = 1; //0: RAM, 1:FRAM
...
extern int recoverable;
	...
	BaseType_t xTaskCreate( TaskFunction_t pxTaskCode,
	                            ...
	                            TaskHandle_t * const pxCreatedTask,
	                            int taskID,
	                            int location) /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
	    {
	    TCB_t *pxNewTCB;
	    BaseType_t xReturn;
...
//	                pxStack = ( StackType_t * ) pvPortMalloc( ( ( ( size_t ) usStackDepth ) * sizeof( StackType_t ) ) ); /*lint !e961 MISRA exception as the casts are only redundant for some ports. */
	            ...
	            if( pxStack != NULL )
	            {
	                ...
	                if(location == INNVM)
	                    pxNewTCB = getTCBAddress(taskID);
                    else{
                        pxNewTCB = getTCBVM(taskID);
//                        pxNewTCB = ( TCB_t * ) pvPortMalloc( sizeof( TCB_t ) ); /*lint !e961 MISRA exception as the casts are only redundant for some paths. */
	                ...
	                if( pxNewTCB != NULL )
	                {
	                    ...
	                    /* Store the function location in the TCB */
	                    pxNewTCB->AddressOfNVMFunction = pxTaskCode;
	                    /* Set the task's ID */
	                    pxNewTCB->taskID = taskID;
	                    if(location == INNVM)
	                ...
	                else
	                {
	                    ...
	                    it again. */ //We now use static allocation
//	                    if(location == INVM)
//	                        vPortFree( pxStack );
	                }
	            }
	        ...
	        //if in VM, the recovery handler records its parameters
	        if(location == INVM)
	            regTaskStart(pxTaskCode, uxPriority, pxNewTCB->uxTCBNumber, pxNewTCB, stopTrack, taskID);


	        return xReturn;
	    }
#else( configSUPPORT_DYNAMIC_ALLOCATION == 1 )
...
#if(configSUPPORT_LENGHTY_TASK == 1)
     BaseType_t xAddTask(void* oldTCB)
{
     TCB_t *pxTCB = (TCB_t*)oldTCB;
     BaseType_t xReturn;
...
/*-----------------------------------------------------------*/
	...
	TCB_t *pxTCB;
		...
		taskENTER_CRITICAL();
		{
			...
			{
			    if(uxCurrentNumberOfTasks > 0)//for recovery handler
			        --uxCurrentNumberOfTasks;
				prvDeleteTCB( pxTCB );
...
BaseType_t xReturn;
    ...
    recoverable = 1;//need to consider recovery after this point
	...
	#else
	{
		...
		xReturn = xTaskCreate(	prvIdleTask,
								...
								&xIdleTaskHandle,
								IDIDLE,
								INVM); /*lint !e961 MISRA exception, justified as it is not a redundant explicit cast to all supported compilers. */
#else
		xReturn = xTaskCreate(  prvIdleTask,
		                                "IDLE", configMINIMAL_STACK_SIZE,
		                                ( void * ) NULL,
	...
	/* Recovery Handler: after a task is started, we start recording the unfinished tasks */
    stopTrack = 0;
	...
	#if ( INCLUDE_vTaskDelete == 1 )
	{
		...
		while( uxDeletedTasksWaitingCleanUp > ( UBaseType_t ) 0U )
		{
			...
			if( xListIsEmpty == pdFALSE )
			{
                ...
                unsigned short TCBNum = pxTCB->uxTCBNumber;
                prvDeleteTCB( pxTCB );
                regTaskEndByIdle(TCBNum);
			}
			else
			{
				mtCOVERAGE_TEST_MARKER();
